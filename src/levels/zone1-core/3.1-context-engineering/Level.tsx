...
export default function FunctionCallingLevel() {
  ...
  return (
    <LevelLayout title="ModelClient 消息协议" levelNumber="1.3" mode={mode} onModeChange={setMode}
      conceptCard={<ConceptCard {...LEVEL_3_1_CONCEPT} />}
      simulation={
        <div className="flex flex-col h-full">
          <div className="flex-1 overflow-y-auto space-y-3 mb-4 min-h-0">
            {chat.length === 0 && (
              <div className="bg-white rounded-xl border border-[#E5E5E5] p-6">
                <h2 className="text-lg font-semibold mb-2">Context 组装观察</h2>
                <p className="text-sm text-[#6B6B6B] mb-2">{mode === 'replay' ? '观察 ContextBuilder 如何按优先级组装 messages，管理 token 预算。' : '调用 LLM API，观察 context manifest 和 auto-compact 机制。'}</p>
          ...
          {quizPhase && !quizPassed && (
            <div className="mb-3 bg-white rounded-xl border border-[#E5E5E5] p-4">
              <h3 className="text-sm font-semibold mb-3">过关测试</h3><p className="text-sm mb-3">{LEVEL_3_1_QUIZ.question}</p>
              <div className="space-y-1.5 mb-3">{LEVEL_3_1_QUIZ.options.map((o, i) => (
              ...
              <Button size="sm" onClick={() => { if (selected === LEVEL_3_1_QUIZ.correctIndex) { setQuizPassed(true); completeLevel('1.3-model-client', mode) } }} disabled={selected === null}>提交</Button>
            </div>
          )}
          {quizPassed && (
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-white rounded-xl border border-[#2DA44E]/30 p-6 text-center">
              ...
              <div className="bg-[#F0F9F2] rounded-lg p-3 mb-3 text-left"><p className="text-xs">{LEVEL_3_1_QUIZ.explanation}</p></div>
