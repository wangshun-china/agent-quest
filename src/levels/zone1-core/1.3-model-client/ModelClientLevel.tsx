...
export default function ModelClientLevel() {
  ...
  const sendLive = async () => {
    ...
    for (let round = 1; round <= 6; round++) {
      ...
      try {
        ...
        // SSE streaming event simulation (for concept demonstration)
        if (round === 1 && chat.length === 0) {
          addEvent(createTraceEvent('observation', 'SSE Stream 事件流', {
            events: 'TextDelta("我来") → TextDelta("查看") → TextDelta("一下") → UsageUpdate(tokens) → ResponseCompleted(finish_reason)',
          }, 'ModelClient.stream() → Iterator[ModelEvent]。实际 fetch 使用 stream=false，但语义一致'))
        }
  ...
  return (
    <LevelLayout title="ModelClient 消息协议" levelNumber="1.3" mode={mode} onModeChange={setMode}
      conceptCard={<ConceptCard {...LEVEL_1_3_CONCEPT} />}
      simulation={
        <div className="flex flex-col h-full">
          ...
          {quizPhase && !quizPassed && (
            <div className="mb-3 bg-white rounded-xl border border-[#E5E5E5] p-4">
              <h3 className="text-sm font-semibold mb-3">过关测试</h3><p className="text-sm mb-3">{LEVEL_1_3_QUIZ.question}</p>
              <div className="space-y-1.5 mb-3">{LEVEL_1_3_QUIZ.options.map((o, i) => (
              ...
              <Button size="sm" onClick={() => { if (selected === LEVEL_1_3_QUIZ.correctIndex) { setQuizPassed(true); completeLevel('1.3-model-client', mode) } }} disabled={selected === null}>提交</Button>
            </div>
          )}
          {quizPassed && (
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-white rounded-xl border border-[#2DA44E]/30 p-6 text-center">
              ...
              <div className="bg-[#F0F9F2] rounded-lg p-3 mb-3 text-left"><p className="text-xs">{LEVEL_1_3_QUIZ.explanation}</p></div>
